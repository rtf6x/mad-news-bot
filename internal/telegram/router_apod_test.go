...
func TestDeliverAPOD_sendsPhotoForImage(t *testing.T) {
	var gotPath, gotPhoto, gotCaption string
	...
	r.deliverAPOD(99, apod.Result{
		Photo:     "https://apod.nasa.gov/pic.jpg",
		Message:   "Red Sun caption",
		MediaType: "image",
	})
	if gotPath != "/bottest-token/sendPhoto" {
		t.Fatalf("path: %q", gotPath)
	}
	if gotPhoto != "https://apod.nasa.gov/pic.jpg" {
		t.Fatalf("photo: %q", gotPhoto)
	}
	if gotCaption != "Red Sun caption" {
		t.Fatalf("caption: %q", gotCaption)
	}
}

func TestDeliverAPOD_sendsMessageForVideo(t *testing.T) {
	var gotPath, gotText, gotPhoto string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = r.ParseForm()
		gotPath = r.URL.Path
